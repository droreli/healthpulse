...
import { hashFileContents, hasProcessedFile, insertSyncLog } from "../ingester/dedup.js";
import type { NormalizedHealthSample, NormalizedSleepSession, NormalizedWorkout } from "../ingester/types.js";
import { insertSamples, insertSleepSessions, insertWorkouts } from "../ingester/service.js";
import { rebuildDerivedTables } from "../pipeline/aggregator.js";
import { computeHRZones } from "../ingester/zones.js";
import type { AppleImportResult } from "./types.js";
...
interface ExportXmlStream {
  stream: Readable;
  completed: Promise<void>;
}

type ExportXmlChildProcess = ChildProcessByStdio<null, Readable, Readable>;
...
export async function importAppleHealthZip(
  ...
  const fileHash = hashFileContents(fileBuffer);
  ...
  const { stream: xmlStream, completed } = await openExportXmlStream(zipPath);
  ...
  db.transaction(() => {
    insertSyncLog(
      ...
      fileHash,
      "apple_xml",
      samplesIngested + workoutsIngested + sleepSessions.length
    );
  })();
...
function buildWorkout(attrs: Record<string, string>): WorkoutBuilder | null {
  ...
  return {
    ...
    avg_heart_rate: null,
    max_heart_rate: null,
    source_name: attrs.sourcename ?? "Apple Health Export"
  };
}
...
function computeCutoffDate(db: Database.Database): string | null {
...
async function openExportXmlStream(zipPath: string): Promise<ExportXmlStream> {
  const child: ExportXmlChildProcess = spawn("unzip", ["-p", zipPath, "apple_health_export/export.xml"], {
  ...
  if (!child.stdout) {
    throw new Error("Failed to read export.xml from zip");
  }

  const completed = waitForChild(child, stderr);
  return {
    stream: child.stdout,
    completed
  };
...
function waitForChild(child: ExportXmlChildProcess, stderr: Buffer[]): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    child.once("close", (code) => {
      ...
      reject(new Error(Buffer.concat(stderr).toString("utf8") || `unzip exited with code ${code}`));
    });
  });
}

function enrichWorkoutsFromHeartRate(db: Database.Database, config: AppConfig): void {
...
function mapSleepValue(rawValue?: string): (typeof SLEEP_VALUE_MAP)[keyof typeof SLEEP_VALUE_MAP] | null {
  if (!rawValue) {
    return null;
  }
