...
export function getDashboardPayload(db: Database.Database, config: AppConfig, range: TimeRange): DashboardPayload {
...
function sleepCard(db: Database.Database, range: TimeRange, comparisonDays: number, stale: boolean): DashboardCardPayload {
  ...
  const baseline = db.prepare(`
    ...
    FROM sleep_sessions
    WHERE date >= ? AND date <= ?
  `).get(previousStartDate, previousEndDate) as { avg: number | null };
  ...
  const trend = computeTrend(latest.total_sleep_hours, baseline.avg);
  return {
    id: "sleep",
    title: "Sleep",
    accent: "#5E5CE6",
...
function metricCard(
  ...
  const baseline = db.prepare(`
    ...
    WHERE metric_name = ?
      AND date >= ?
      AND date <= ?
  `).get(options.metricName, previousStartDate, previousEndDate) as { avg: number | null };
  ...
  const trend = computeTrend(latest.value, baseline.avg, options.preferLower);
  return {
    id: options.id,
    title: options.title,
...
function stepsCard(db: Database.Database, range: TimeRange, comparisonDays: number, stale: boolean): DashboardCardPayload {
  const start = rangeStart(range);
  const currentStartDate = format(start, "yyyy-MM-dd");
  const previousStartDate = format(subDays(start, comparisonDays), "yyyy-MM-dd");
  const previousEndDate = format(subDays(start, 1), "yyyy-MM-dd");
  ...
  const trend = computeTrend(latest.sum_value, baseline.avg);
  return {
    id: "steps",
    title: "Steps",
    accent: "#30D158",
...
function workoutCard(db: Database.Database, range: TimeRange, comparisonDays: number, stale: boolean): DashboardCardPayload {
  const start = rangeStart(range);
  const currentStartDate = format(start, "yyyy-MM-dd");
  ...
  const previous = db.prepare(`
    ...
    FROM workouts
    WHERE substr(start_time, 1, 10) >= ? AND substr(start_time, 1, 10) <= ?
  `).get(previousStartDate, previousEndDate) as { count: number };
  ...
  const trend = computeTrend(current.count, previous.count);
  ...
  return {
    ...
    secondary,
    trend: trend.trend,
    deltaLabel: trend.deltaPct !== null
      ? `${trend.deltaPct > 0 ? "+" : ""}${trend.deltaPct}%`
      : `${current.count - previous.count >= 0 ? "+" : ""}${current.count - previous.count}`,
    deltaBasis: `vs previous ${windowLabel(comparisonDays)}`,
    sparkline,
    status: stale ? "stale" : "ok"
  };
